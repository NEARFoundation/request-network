use request_proxy::RequestProxyContract;
...
lazy_static_include::lazy_static_include_bytes! {
   REQUEST_PROXY_BYTES => "../out/main.wasm"
}

pub fn init_request_proxy(
    initial_balance: u128
) -> (UserAccount, ContractAccount<RequestProxyContract>, UserAccount) {
    let genesis = GenesisConfig::default();
    let root = init_simulator(Some(genesis));

    let request_proxy = deploy!(
        contract: RequestProxyContract,
